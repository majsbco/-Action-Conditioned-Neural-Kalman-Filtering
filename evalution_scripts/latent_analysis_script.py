...
def analyze_latent_space_pro():
    ...
    print(f"--- 启动 PCA 单维度平滑度深度评估 | 设备: {device.type} ---")
    ...
    Z = np.concatenate(all_latents, axis=0) # (Frames, 32)

    # --- 核心量化逻辑：单维度分析 ---
    max_k = 10
    pca = PCA(n_components=max_k)
    Z_pca = pca.fit_transform(Z) # (Frames, 10)
    
    indiv_jitter = [] # 每一维独立的抖动 (二阶差分)
    indiv_velocity = [] # 每一维独立的步长 (一阶差分)
    ...
    # 子图 1: 每一维度的独立抖动程度 (衡量平滑度)
    ax1.bar(range(1, max_k + 1), indiv_jitter, color='salmon', edgecolor='black', alpha=0.8)
    ax1.set_title("Individual Smoothness: Jitter per PC Axis", fontsize=14)
    ...
    # 子图 2: 归一化平滑度 (方差 / 抖动) -> 衡量该维度是否“高质”
    # 如果一个维度方差很大但抖动很小，说明它极其优质
    quality_score = pca.explained_variance_ratio_ / (np.array(indiv_jitter) + 1e-9)
    ax2.plot(range(1, max_k + 1), quality_score, 'o-', color='green', linewidth=2)
    ax2.fill_between(range(1, max_k + 1), quality_score, color='green', alpha=0.1)
    ...
    plt.tight_layout()
    save_p = os.path.join(PROJECT_ROOT, 'pca_dimension_smoothness_report.png')
    plt.savefig(save_p, dpi=150)
    print(f"\n量化分析完成。独立评估报告已保存至: {save_p}")
    plt.show()

if __name__ == '__main__':
    analyze_latent_space_pro()